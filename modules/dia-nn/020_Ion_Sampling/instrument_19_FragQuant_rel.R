init <- function() {
  ...
  type <- 'plot'
  box_title <- 'Fragment intensities by MZ normalized to first run'
  help_text <- 'Plotting the individual fragment intensities for top 10 most abundant intersected precursors in D8. Experiments are normalized to the first experiment. '
  source_file <- 'report'
  ...
  .plotdata <- function(data, input) {
    ...
    for(i in unique(plotdata$Raw.file)){
      ...
      if(count == 1){
        ...
        frag_df_final_hold$Intensity <-frag_df_final_hold$Intensity/median(frag_df_final_hold$Intensity,na.rm=T)
        frag_df_final <- frag_df_quant
        frag_df_final$Intensity <- log2(frag_df_final_hold$Intensity/frag_df_final_hold$Intensity)

      }else{
        frag_df_quant$Intensity <- frag_df_quant$Intensity/median(frag_df_quant$Intensity,na.rm = T)
        frag_df_quant$Intensity <- log2(frag_df_quant$Intensity/frag_df_final_hold$Intensity)
    ...
    frag_df_final$Frag <- round(frag_df_final$Frag/20)*20
  ...
  .plot <- function(data, input) {
    ...
    ggplot(plotdata,aes(y = Intensity,x = Frag)) + geom_line()+
      facet_wrap(~Run, nrow = 1)+coord_cartesian(ylim = c(-2,2))+
      ylab('log2(Normalized Fragment Intensity)') +xlab('Fragment M/Z')+rremove("legend")  + 
