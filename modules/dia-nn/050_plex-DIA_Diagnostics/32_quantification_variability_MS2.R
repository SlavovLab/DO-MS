init <- function() {
  ...
  type <- 'plot'
  box_title <- 'MS2 Quantification Variability'
  ...
  .plotdata <- function(data, input) {
    ...
    plotdata <- data()[['report']][,c('Raw.file', 'Precursor.Quantity', 'Precursor.Id', 'Label', 'Protein.Group','Stripped.Sequence','Precursor.Charge')]
    ...
    #plotdata <- plotdata[,c('Raw.file', 'Precursor.Quantity', 'Precursor.Id', 'Label', 'Protein.Group','Stripped.Sequence','Precursor.Charge')]
    ...
    plotdata <- plotdata[plotdata$Precursor.Quantity>0,]
    ...
    plotdata <- plotdata %>% dplyr::group_by(Raw.file, seqcharge) %>% dplyr::mutate("Precursor.Quantity"=Precursor.Quantity/mean(Precursor.Quantity, na.rm=T)) %>% ungroup()
    
    # Normalize Precursor.Quantity based with the median of each cell
    plotdata <- plotdata %>% 
      dplyr::group_by(Raw.file, Label) %>% 
      dplyr::mutate("Precursor.Quantity.Norm" = Precursor.Quantity/median(Precursor.Quantity, na.rm=T)) %>% dplyr::ungroup()
    ...
    plotdata <- plotdata %>% 
      dplyr::group_by(Raw.file, Label, Protein.Group) %>% 
      dplyr::summarise("cv" = sd(Precursor.Quantity.Norm)/mean(Precursor.Quantity.Norm))
