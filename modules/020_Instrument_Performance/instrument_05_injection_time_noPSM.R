init <- function() {
  ...
  help_text <- 'Plotting distribution of injection times for MS2 events that did not result in a PSM.'
  ...
  .plotdata <- function(data, input) {
    plotdata <- data()[[source_file]][,c('Raw.file', 'Ion.injection.time', 'Sequence')]
