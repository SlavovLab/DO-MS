...
init <- function() {
  return(list(
    ...
    help='Plotting the distrution of elution profile widths at half the maximum
    intensity value for each peak.',
...
testModule <- function(input, output, session, data) {
  ...
  output$plot <- renderPlot({
    ...
    # Plot:
    histdata <- data.loaded[,c("Raw.file","Retention.length..FWHM.")]
